...
#define TRANS_COLOR makecol(255, 0, 255)  
...
#define SHIP_HITBOX_WIDTH 20
#define SHIP_HITBOX_HEIGHT 10


BITMAP *enemy_sprites[3];
...
typedef struct {
    ...
    BITMAP *enemy_shot_frames[NUM_SHOT_FRAMES];  // Tableau pour les frames des tirs ennemis
} Shot;
...
typedef struct {
    ...
    int current_frame;
    int frame_counter;
    int is_enemy_hit;  // Pour distinguer entre explosion de tir joueur et ennemi
    BITMAP *hit_frames[NUM_HIT_FRAMES];
    BITMAP *enemy_hit_frames[8];  // 8 frames pour les explosions des tirs ennemis
} HitEffect;
...
// Fonction pour vérifier si un point est à l'intérieur d'un ovale
// Fonction pour vérifier si un point est à l'intérieur d'un ovale
...
BITMAP* load_and_process_sprite(const char* filename, int scale_factor) {
    BITMAP* original = load_bitmap(filename, NULL);
    if (!original) return NULL;

    // Créer une bitmap redimensionnée si nécessaire
    BITMAP* scaled;
    if (scale_factor > 1) {
        scaled = create_bitmap(original->w * scale_factor, original->h * scale_factor);
        clear_to_color(scaled, TRANS_COLOR);

        // Redimensionner l'image (méthode simple)
        stretch_blit(original, scaled, 0, 0, original->w, original->h,
                    0, 0, scaled->w, scaled->h);
...
void load_enemy_hit_frames(HitEffect *effects) {
    char filename[256];

    // Supposons que NUM_ENEMY_HIT_FRAMES est défini comme 8
    for (int i = 0; i < 8; i++) {
        sprintf(filename, "SHOT2/shot2_exp%d.bmp", i + 1);
        BITMAP *frame = load_and_process_sprite(filename, 3); // Facteur d'échelle 3x pour les explosions
        if (!frame) {
        ...
        for (int j = 0; j < MAX_SHOTS; j++) {
            effects[j].enemy_hit_frames[i] = frame;
        }
    }
}

void load_enemy_shot_frames(Shot *shots) {
    char filename[256];

    for (int i = 0; i < NUM_SHOT_FRAMES; i++) {
        sprintf(filename, "SHOT2/shot2_%d.bmp", i + 1);
        BITMAP *frame = load_and_process_sprite(filename, 2); // Facteur d'échelle 2x
        if (!frame) {
            allegro_message("Erreur chargement %s", filename);
            exit(1);
        }

        // Assigner la même frame à tous les shots ennemis
        for (int j = 0; j < MAX_SHOTS; j++) {
            shots[j].enemy_shot_frames[i] = frame; // Utiliser un tableau pour les frames des tirs ennemis
        }
    }
}
...
void load_shot_frames(Shot *shots) {
    ...
    for (int i = 0; i < NUM_SHOT_FRAMES; i++) {  // Commencer à 0 au lieu de 1
...
int check_collisions(Shot shots[], Enemy enemies[], Spaceship *ship, Bonus *bonus, HitEffect effects[]) {
    ...
    float oval_a = SHIP_HITBOX_WIDTH / 2.0+ 35;  // Demi-axe horizontal
    float oval_b = SHIP_HITBOX_HEIGHT / 2.0 + 20; // Demi-axe vertical

    // Décaler le centre de l'ovale de 10 pixels vers la droite
    float oval_x = ship->x + 70;  // Centre de l'ovale (x) - décalé de 10 pixels
    float oval_y = ship->y;       // Centre de l'ovale (y)
    ...
    if (ship->immunity_timer <= 0) {
        for (int i = 0; i < MAX_ENEMIES; i++) {
            if (enemies[i].active) {
                ...
                float enemy_left = enemies[i].x - enemy_size/2;
                float enemy_right = enemies[i].x + enemy_size/2;
                float enemy_top = enemies[i].y - enemy_size/2;
                float enemy_bottom = enemies[i].y + enemy_size/2;

                // Vérifier si un des points de l'ennemi est dans l'ovale
                if (point_in_oval(enemy_left, enemy_top, oval_x, oval_y, oval_a, oval_b) ||
                    point_in_oval(enemy_right, enemy_top, oval_x, oval_y, oval_a, oval_b) ||
                    point_in_oval(enemy_left, enemy_bottom, oval_x, oval_y, oval_a, oval_b) ||
                    point_in_oval(enemy_right, enemy_bottom, oval_x, oval_y, oval_a, oval_b) ||
                    point_in_oval(enemies[i].x, enemies[i].y, oval_x, oval_y, oval_a, oval_b)) {
    ...
    for (int i = 0; i < MAX_SHOTS; i++) {
        if (shots[i].active) {
            if (!shots[i].is_enemy_shot) {
                ...
                if (point_in_oval(shots[i].x, shots[i].y, oval_x, oval_y, oval_a, oval_b)) {
                    // Activer l'effet d'impact
                    effects[i].x = shots[i].x;
                    effects[i].y = shots[i].y;
                    effects[i].active = 1;
    ...
    for (int i = 0; i < MAX_SHOTS; i++) {
        if (shots[i].active) {
            if (shots[i].is_enemy_shot) {
                ...
                if (shots[i].current_frame < NUM_SHOT_FRAMES) {
                    BITMAP *frame = shots[i].enemy_shot_frames[shots[i].current_frame];
                    if (frame) {
                        int draw_x = shots[i].x - frame->w;
...
void draw_hit_effects(HitEffect effects[], BITMAP *buffer) {
    for (int i = 0; i < MAX_SHOTS; i++) {
        if (effects[i].active) {
            if (effects[i].is_enemy_hit) {
                // Explosion de tir ennemi
                if (effects[i].current_frame < 8) {  // 8 frames pour les explosions ennemies
                    BITMAP *frame = effects[i].enemy_hit_frames[effects[i].current_frame];
...
// Fonction pour retourner horizontalement un sprite
BITMAP* flip_sprite_horizontal(BITMAP* original) {
    BITMAP* flipped = create_bitmap(original->w, original->h);
    clear_to_color(flipped, TRANS_COLOR);

    for (int y = 0; y < original->h; y++) {
        for (int x = 0; x < original->w; x++) {
            int color = getpixel(original, x, y);
            putpixel(flipped, original->w - x - 1, y, color);
        }
    }
...
// Modifier la fonction draw_enemies pour utiliser les sprites
void draw_enemies(Enemy enemies[], BITMAP *buffer, BITMAP *enemy_sprites[]) {
    for (int i = 0; i < MAX_ENEMIES; i++) {
        if (enemies[i].active) {
            if (enemies[i].is_boss) {
                ...
                int sprite_index = enemies[i].level - 1;
                if (sprite_index >= 0 && sprite_index < 3 && enemy_sprites[sprite_index]) {
                    BITMAP *enemy_sprite = enemy_sprites[sprite_index];

                    // Dessiner le sprite centré sur la position de l'ennemi
                    draw_sprite(buffer, enemy_sprite,
                               (int)enemies[i].x - enemy_sprite->w/2,
                               (int)enemies[i].y - enemy_sprite->h/2);
                    ...
                    // Fallback au dessin original si le sprite n'est pas disponible
                    int color;
...
void draw_ship(Spaceship *ship, BITMAP *buffer, BITMAP *ship_sprite) {
    ...
    float oval_a = SHIP_HITBOX_WIDTH / 2.0 + 35;
    float oval_b = SHIP_HITBOX_HEIGHT / 2.0 + 20;

    // Décaler le centre de l'ovale de 10 pixels vers la droite
    float oval_x = ship->x + 70;
    float oval_y = ship->y;

    //ellipse(buffer, oval_x, oval_y, oval_a, oval_b, makecol(255, 0, 0));
...
int main() {
    ...
    BITMAP *ship_sprite = load_and_process_sprite("Ship.bmp", 1);
    ...
    // Tableau pour stocker les sprites des ennemis pour chaque niveau
    BITMAP *enemy_sprites[3];
    ...
    // Niveau 1 - Ship4
    temp_sprite = load_and_process_sprite("Ship4.bmp", 1);
    if (temp_sprite) {
        enemy_sprites[0] = flip_sprite_horizontal(temp_sprite);
        destroy_bitmap(temp_sprite);
    } else {
        enemy_sprites[0] = NULL;
    }

    // Niveau 2 - Ship3
    temp_sprite = load_and_process_sprite("Ship3.bmp", 1);
    if (temp_sprite) {
        enemy_sprites[1] = flip_sprite_horizontal(temp_sprite);
        destroy_bitmap(temp_sprite);
    } else {
        enemy_sprites[1] = NULL;
    }

    // Niveau 3 - Ship2
    temp_sprite = load_and_process_sprite("Ship2.bmp", 1);
    if (temp_sprite) {
        enemy_sprites[2] = flip_sprite_horizontal(temp_sprite);
        destroy_bitmap(temp_sprite);
    } else {
        enemy_sprites[2] = NULL;
    }
    ...
    for (int i = 0; i < 3; i++) {
        if (!enemy_sprites[i]) {
            char filename[20];
            sprintf(filename, "Ship%d.bmp", 4 - i);
            allegro_message("Erreur: Impossible de charger %s", filename);
            // Continuer malgré l'erreur, on utilisera le dessin par défaut
        }
    }
    ...
    while (!game_over && !key[KEY_ESC]) {
        ...
        draw_enemies(enemies, buffer, enemy_sprites);
    ...
    for (int i = 0; i < 3; i++) {
        if (enemy_sprites[i]) destroy_bitmap(enemy_sprites[i]);
    }
