...
int main() {
    ...
    while (!quitter_jeu) {
        ...
        while (!partie_terminee && !key[KEY_ESC]) {
            ...
            gerer_niveau(&timer_niveau, &niveau_actuel, &generer_ennemis, &niveau_termine,&boss_genere, ennemis, tampon, niveau_choisi, &score,&vaisseau, &partie_terminee, &ennemis_actifs);
